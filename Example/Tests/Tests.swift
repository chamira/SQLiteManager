...
COMPANY(ID INT PRIMARY KEY NOT NULL,NAME TEXT NOT NULL, AGE INT NOT NULL,ADDRESS CHAR(50), SALARY REAL)
"""
		let ret = try! database.query(query)

		XCTAssertEqual(SQLITE_OK, ret.SQLiteSatusCode,
                       "Table is not created successfully SQLStatusCode:\(ret.SQLiteSatusCode)")
	...
	func testBatchQueries() {
		...
		let insertSync:(_ queries: [String]) -> Void = { (queries) -> Void in
			let res = try! database.query(queries)
			XCTAssert(res.results.count == queries.count, "queries count \(queries.count) ≠ \(res.results.count)")
			print("Time taken to execute \(queries.count) queries", res.timeTaken)
		}
