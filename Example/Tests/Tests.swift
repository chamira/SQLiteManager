	...
	override func spec() {
		...
		describe("SQLite.manager() bind queries") {
			...
			it ("INSERT SUCCESS") {
				...
				let result = try! database.bindQuery(sqlStatement: "INSERT INTO 'tb_user' (first_name, last_name, username, date_of_birth) VALUES (?,?,?,?)",
				                                     bindValues: ["Chameera" as NSString,"Fernando" as NSString,"some_user_name" as NSString, NSNumber(double: dob.timeIntervalSince1970)])
			...
			it ("UPDATE SUCCESS") {
				let result = try! database.bindQuery(sqlStatement: "UPDATE 'tb_user' SET first_name = ?, last_name = ? WHERE first_name = ?", bindValues: ["Chamira" as NSString,"Fernando" as NSString, "Chameera" as NSString])
				expect(SQLITE_OK) == result.SQLiteSatusCode
				expect(result.affectedRowCount) == 1
				expect(result.results).to(beFalsy())
			}
			...
			it ("SELECT SUCCESS") {
				let result = try! database.bindQuery(sqlStatement: "SELECT first_name, date_of_birth as dob from 'tb_user' where first_name=?", bindValues: ["Chamira" as NSString])
				expect(SQLITE_OK) == result.SQLiteSatusCode
				expect(result.affectedRowCount) == 1
			...
			it ("DELETE SUCCESS") {
				let result = try! database.bindQuery(sqlStatement: "DELETE FROM 'tb_user' WHERE first_name = ?",bindValues: ["Chamira" as NSString])
				expect(SQLITE_OK) == result.SQLiteSatusCode
